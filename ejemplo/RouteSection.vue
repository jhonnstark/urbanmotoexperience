  ...
  <section id="route" class="route-section" aria-labelledby="route-title">
    <div class="route-shell">
      <div class="route-copy">
        <p class="route-eyebrow reveal reveal-1">{{ $t?.('route.eyebrow') ?? 'The Route' }}</p>
        <div class="eyebrow-line reveal reveal-2" aria-hidden="true"></div>

        <h2 id="route-title" class="route-title reveal reveal-3">
          {{ $t?.('route.title') ?? 'Main Urban Route' }}
        </h2>

        <p class="route-subtitle reveal reveal-4">
          {{
            $t?.('route.subtitle') ??
            'A relaxed urban ride combining architecture, history, parks, and iconic city views.'
          }}
        </p>
        ...
        <ol class="route-list" aria-label="Urban Moto route stops">
          ...
          >
            <img class="stop-thumb" :src="stop.image" :alt="stop.title" />
      ...
      <div class="map-wrap reveal-map">
        <div class="map-card" aria-label="Illustrated route map">
          ...
          <img class="map-logo" src="/images/logo/urban-moto-badge.png" alt="Urban Moto Experience" />
          ...
          >
            ...
            <img v-if="point.image" :src="point.image" :alt="point.label" />
          </div>
        </div>
...
const stops: Stop[] = [
  {
    id: 1,
    title: 'Parque España',
    description: 'We start at this vibrant spot in the heart of Condesa.',
    image: '/images/route/parque-espana.jpg',
  },
  {
    id: 2,
    title: 'Ángel de la Independencia',
    description: 'Ride down Reforma Avenue to the most iconic monument in the city.',
    image: '/images/route/angel.jpg',
  },
  {
    id: 3,
    title: 'Chapultepec Lake',
    description: 'Enjoy the green views and a relaxing stop by the lake.',
    image: '/images/route/chapultepec-lake.jpg',
  },
  {
    id: 4,
    title: 'Museo de Antropología',
    description: 'Explore one of Mexico’s most important cultural landmarks.',
    image: '/images/route/museo-antropologia.jpg',
  },
  {
    id: 5,
    title: 'Palacio de Bellas Artes',
    description: 'Admire the architecture of this stunning historic building.',
    image: '/images/route/bellas-artes.jpg',
  },
  {
    id: 6,
    title: 'Monumento a la Revolución',
    description: 'We finish at this impressive icon of Mexican history.',
    image: '/images/route/revolucion.jpg',
  },
]

const mapPoints: MapPoint[] = stops.map((stop) => ({
  id: stop.id,
  label: stop.title,
  image: stop.image,
}))
...
const stats: Stat[] = [
  { icon: '◷', label: 'Duration', value: '3 – 4 hours' },
  { icon: '⌁', label: 'Distance', value: 'Approx. 25 km' },
  { icon: '🏍', label: 'Ride Type', value: 'Urban & Scenic' },
  { icon: '♙', label: 'Group Size', value: '2 – 8 people' },
]
</script>
...
.stop-thumb {
  ...
  background:
    radial-gradient(circle at 40% 35%, #eaf4e5, #a6c4dc 48%, #6b8e54);
